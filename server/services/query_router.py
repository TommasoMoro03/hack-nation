...
- Finance API: For current prices, quantitative data, predictions, market data, stock prices
...
"""
    ...
    def _handle_rag_query(self, question: str, selected_documents: Optional[List[str]], start_time: float) -> RouterResult:
    ...
    def _handle_mixed_query(self, question: str, selected_documents: Optional[List[str]], analysis: QueryAnalysis, start_time: float) -> RouterResult:
        """Handle mixed query requiring both sources"""
        try:
            # Get RAG answer
            rag_result = self.rag_service.process_query(question, selected_documents)
            
            # Get finance data if symbols are mentioned
            finance_data = {}
            if analysis.symbols:
                finance_data = self.finance_service.get_multiple_stocks(analysis.symbols)
            
            # Combine answers
            combined_answer = self._combine_answers(rag_result.answer, finance_data, question)
            ...
            return RouterResult(
                ...
                data={
                    ...
                    'finance_symbols': list(finance_data.keys()) if finance_data else []
                },
                processing_time=processing_time
            )
    ...
    def _generate_finance_answer(self, question: str, stock_data: Dict, analysis: QueryAnalysis) -> str:
        ...
        try:
            ...
            for symbol, data in stock_data.items():
                stock_info.append(f"{symbol}: ${data.current_price:.2f} ({data.change:+.2f}, {data.change_percent:+.2f}%)")
...
"""
    ...
    def _combine_answers(self, rag_answer: str, finance_data: Dict, question: str) -> str:
